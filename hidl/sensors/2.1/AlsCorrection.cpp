...
using aidl::vendor::lineage::xiaomi_als::AreaRgbCaptureResult;
using aidl::vendor::lineage::xiaomi_als::IAreaCapture;
...
void AlsCorrection::init() {
    ...
    max_brightness = get(BRIGHTNESS_DIR "max_brightness", 4095.0);
    ALOGI("Display maximums: R=%d G=%d B=%d W=%d, Brightness=%d",
...
void AlsCorrection::correct(float& light) {
    ...
    if ((now - last_update) > ms2ns(500)) {
        ...
        ALOGI("Capture skipped because interval not expired at %ld", now);
    }

    float r = screenshot.r / 255, g = screenshot.g / 255, b = screenshot.b / 255;
    ALOGI("Screen Color Above Sensor: %f, %f, %f", r, g, b);
    ...
    ALOGI("Corrected reading: %f", light);
}
